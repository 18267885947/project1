...
class OrderPlaceView(LoginRequiredMixin,View):
    ...
    def post(self,request):
        ...
        for sku_id in sku_ids:
            ...
            #动态给sku增加属性count，amount，保存的商品的数量和小计
            sku.count=count
            sku.amount=amount
            #追加
            skus.append(sku)
...
class OrderCommitView(View):#订单创建
    @transaction.atomic
    def post(self,request):
        '''订单创建'''
        user=request.user
        ...
        if not all([addr_id,pay_method,sku_ids]):#校验参数
        ...
        #组织参数
        #订单id:20171122183230+用户id
        order_id=datetime.now().strftime('%Y%m%d%H%M%S')+str(user.id)
        transit_price=10#运费
        total_count=0#总数目和总金额
        total_price=0
        #设置事务保存点
        ...
        try:
            ...
            for sku_id in sku_ids:#获取商品的信息
                try:
                    ...
                    sku=GoodsSKU.objects.select_for_update().get(id=sku_id)#select_for_update添加悲观锁
                except:
